...
void step(uint32_t field[WC]) {
    ...
    for (uint8_t y = 1; y < HC; y++) {
        ...
        if (CELL_LIVE(prev_state[0], (y - 1))) alive_neighbors++;
        if (CELL_LIVE(prev_state[1], (y - 1))) alive_neighbors++;
        if (CELL_LIVE(field[x + 1], (y - 1))) alive_neighbors++;
        if (CELL_LIVE(field[x + 1], (y))) alive_neighbors++;
        if (CELL_LIVE(field[x + 1], (y + 1))) alive_neighbors++;
        if (CELL_LIVE(prev_state[1], (y + 1))) alive_neighbors++;
        if (CELL_LIVE(prev_state[0], (y + 1))) alive_neighbors++;
        if (CELL_LIVE(prev_state[0], (y))) alive_neighbors++;
...
#    ifdef EGDE_KILL
        for (uint8_t x = 0; x < WC; x++)
            for (uint8_t y = 0; y < HC; y++)
                if (y == 0 || y == HC - 1 || x == 0 || x == WC - 1) CLR_CELL(field[x], y);
#    endif
    }
}
        ...
        if (csc->level == 1) // Ring +1
        {
            for (int8_t xo = -1; xo <= 1; xo++)
                for (int8_t yo = -1; yo <= 1; yo++) {
                    if ((rand >> nbit % 16) & 1) SET_CELL(field[csc->x + xo + MATRIX_POS_X], (csc->y + yo + MATRIX_POS_Y));
                    nbit++;
                }
...
void conway_process_record_user(uint16_t keycode, keyrecord_t *record) {
    conway_calc_spawn_pos(keycode, record);
